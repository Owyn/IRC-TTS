...
;MsgBox % TTS(Voice, "GetVoices") ; get list of your voices
;ExitApp

VoiceRate := 0
VoiceVolume := 30
VoicePitch := 0
VoiceName_RU_F := "IVONA 2 Tatyana"
VoiceName_RU_M := "IVONA 2 Maxim OEM"
VoiceName_EN_F := "IVONA 2 Salli"
VoiceName_EN_M := "IVONA 2 Justin"

TheVoice := TTS_CreateVoice(VoiceName_RU_F, VoiceRate, VoiceVolume, VoicePitch)
...
Loop
{
	...
	if WinExist("ntScript")
	{
		ControlGetText, MsgText, Static1, ntScript ; ~700 symbols max 
		if(SubStr(MsgText, 1, 1) == "!")
		{
			StoppedAT := 2
			Cmd := True
		}
		...
		Loop
		{
			PosRu := RegexMatch(MsgText, "S)[а-яА-ЯёЁ]",,StoppedAT)
			PosEn := RegexMatch(MsgText, "S)[a-zA-Z]",,StoppedAT)
			if(PosRu == 0)
			{
				PosRu := StrLen(MsgText)+1
				TimeToStop := true
			}
			if(PosEn == 0) ; put "else if" for reading text consisting of only numbers and special symbols, switch if places to switch language
			{
				PosEn := StrLen(MsgText)+1
				TimeToStop := true
			}
			
			if(PosRu < PosEn) ; Russian comes first
			{
				PartToSpeak := SubStr(MsgText, StoppedAT, PosEn-PosRu)
				StoppedAT := PosEn
				if(Cmd)
				{
					TTS(TheVoice, "SetVoice", VoiceName_RU_M)
				}
				else
				{
					TTS(TheVoice, "SetVoice", VoiceName_RU_F)
				}
			}
			else
			{
				PartToSpeak := SubStr(MsgText, StoppedAT, PosRu-PosEn)
				StoppedAT := PosRu
				if(Cmd)
				{
					TTS(TheVoice, "SetVoice", VoiceName_EN_M)
				}
				else
				{
					TTS(TheVoice, "SetVoice", VoiceName_EN_F)
				}
			}
			TTS(TheVoice, "SpeakWait", PartToSpeak)
			if(TimeToStop)
			{
				break
			}
		...
		WinClose, ntScript
		Sleep 3000 ; rest time between texts in ms
	}
	Sleep 1000 ; time between looking for a new text
}
